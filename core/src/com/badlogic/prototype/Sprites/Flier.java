...
public class Flier extends Sprite
{
    //states
    public enum State {DEATH, IDLE, HURT, MOVE, SD, SHOOT}
    ...
    private Array<TextureAtlas.AtlasRegion> sdFrames;
    private Array<TextureAtlas.AtlasRegion> shootFrames;
    //other
    ...
    public Flier(Level1 screen)
    {
        ...
        movingRight=true;
        //anims and frames
        textureAtlas = new TextureAtlas(Gdx.files.internal("Enemies/Drone1/Drone1.atlas"));
        deathFrames=textureAtlas.findRegions("death");
        deathAnim = new Animation(1/9f,deathFrames);
        idleFrames=textureAtlas.findRegions("idle");
        idleAnim = new Animation(1/4f,idleFrames);
        hurtFrames=textureAtlas.findRegions("hurt");
        hurtAnim = new Animation(1/3f,hurtFrames);
        moveFrames=textureAtlas.findRegions("move");
        moveAnim = new Animation(1/4f,moveFrames);
        sdFrames=textureAtlas.findRegions("sd");
        sdAnim = new Animation(1/4f,sdFrames);
        shootFrames=textureAtlas.findRegions("shoot");
        shootAnim = new Animation(1/12f,shootFrames);
    ...
    public TextureRegion getFrame(float elapsedTime, float dt)
    {
        ...
        switch(currentState)
        {
            ...
            case MOVE:
                ...
                break;
            case SD:
                region = ((TextureRegion) sdAnim.getKeyFrame(elapsedTime, false));
                break;
            case SHOOT:
                region = ((TextureRegion) shootAnim.getKeyFrame(elapsedTime, false));
                break;
            default:
    ...
    public State getState(float elapsedTime)
    {
        if(isAction)
        {
            switch(currentState)
            {
                ...
                case SHOOT:
                    return shootAnim.getKeyFrame(elapsedTime)==shootFrames.get(shootFrames.size-1)?State.IDLE:State.SHOOT;
                case SD:
                    return sdAnim.getKeyFrame(elapsedTime)==sdFrames.get(sdFrames.size-1)?State.IDLE:State.SD;
                case DEATH:
    ...
    //what the fuck does "sd" mean
    public void sd()
    {
        if(!IsAction())
        {
            isAction=true;
            currentState=State.SD;
        }
    }
    ...
    public void defineFlier()
    {
        ...
        shape.setAsBox(16/Prototype.PPM,16/Prototype.PPM); //unsure how to account for change in dimensions with death anim
        //fDef.filter.categoryBits leaving here for posterity, unsure how you guys want to account for bits in this stuff
        fDef.shape=shape;
        fDef.friction=0; //bro is flying, I don't think friction will be an issue
